...
func DeleteMountPoint(containerName string) error {
	mntURL := fmt.Sprintf(MntUrl, containerName)
	_, err := exec.Command("umount","-lr", mntURL).CombinedOutput()
	if err != nil {
...
func DeleteMountPointWithVolume(volumeURLs []string, containerName string) error {
	...
	if _, err := exec.Command("umount","-rl", containerUrl).CombinedOutput(); err != nil {
		log.Errorf("Umount volume %s failed. %v", containerUrl, err)
		return err
	}

	if _, err := exec.Command("umount","-rl", mntURL).CombinedOutput(); err != nil {
...
func DeleteWriteLayer(containerName string) {
	writeURL := fmt.Sprintf(WriteLayerUrl, containerName)
	log.Info("start to delete writeLayer ",writeURL)
